...
- 방법
  - 컴퓨존 > '데크스탑, 서버' 탭을 기준
    - 제품의 링크를 타고 들어가서 먼저 페이지 소스에 스펙 정보가 있는지 확인
      1. DIV pddt_info_area > DIV recom_L 이 존재하는지 확인한다.
         - 만약 존재한다면 정보를 가져온다.
      2. 찾을 수 없는 경우 광고 이미지를 가져온다.
      3. 광고 이미지에서 텍스트를 추출한다.
      4. 얻어진 텍스트 형태의 스펙 정보를 엔터티 단위로 분리
         - 처음에는 월등한 mecab를 사용하려고 했지만, 기본적으로 엔터티 인식을 하지 않았다.
         - 그래서 대안으로 Komoran이나 khaiii중 하나를 선택하기로 했다.
           - https://iostream.tistory.com/144
           - 위 분석 결과를 바탕으로 komoran을 선택했다.
      5. 부품 정보를 추출한다.
...
- 향후 개선 방향
  - 완제품 판매가와 실제 개별 부품을 모아 직접 조립하는 비용으로부터 마진율을 계산해서 알려주는 기능을 추가할 예정
...
- 고민 사항
  ...
  - 불완전한 데이터
    ...
    2. 서버 컴퓨터나 개별 부품에 대한 필터링 방법
       - 사실 해결이라기 보다는 타협이라고 하면 타협일 수는 있지만, 요새는 베어본으로 나오는 pc조차도 사무용으로 쓰기에는 전혀 문제없기 때문에 포함하는 방향으로 설정
