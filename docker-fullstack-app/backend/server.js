...
db.pool.query(`CREATE TABLE main_comments (
  ...
  if (err) {
    ...
    console.log("main_comments table:", results);
    db.pool.query(`CREATE TABLE sub_comments (
      ...
      if (err) {
        ...
        console.log("sub_comments table:", results);
...
app.patch(`/api/comment-update/:id`, function (req, res) {
  ...
  db.pool.query(`SELECT * FROM main_comments WHERE id=("${req.params.id}")`,
    (err, results) => {
      if (err) {
        // id로 불러오지 못함
        console.log("Failed to load with id:", err)
        return res.status(500).json({
          statusCode: res.statusCode,
          message: err,
        });
...
// 서버 열기
app.listen(PORT, () => {
  console.log(`Started backend server at PORT:${PORT}`);
});