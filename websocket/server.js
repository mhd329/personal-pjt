...
server.on("connection", (socket) => { // 연결
    ...
    const nowRequestIp = socket.handshake.address; // 클라이언트의 IP주소
    socket.on("fromFront", async (data) => { // 데이터 받아오기
        console.log("웹소켓 서버가 받은 데이터:", data); // 받은 데이터 확인
        const PrevRequestTime = await redisClient.get(nowRequestIp); // 클라이언트가 기존에 요청했던 시간
        ...
        if (nowRequestTime - PrevRequestTime < requestLimitInterval) { // 시간 내에 요청이 너무 많은 상태
            ...
            await redisClient.set(nowRequestIp, nowRequestTime);
            ...
            setTimeout(async () => { // 마지막 요청 시간 초기화
                await redisClient.set(nowRequestIp, 0);
